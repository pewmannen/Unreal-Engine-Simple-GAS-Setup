...
/**
 PHSLog.h
 Centralized log categories for PHS_Abilities.
 Usage:
 ...
 To port to a new project: rename the PHS_API macro to match
 the new project's module export macro, then update the log category names
 if desired. Everything else is self-contained.
 */

PHS_API DECLARE_LOG_CATEGORY_EXTERN(LogPHS,        Log, All);
PHS_API DECLARE_LOG_CATEGORY_EXTERN(LogPHSAbility, Log, All);
PHS_API DECLARE_LOG_CATEGORY_EXTERN(LogPHSInput,   Log, All);